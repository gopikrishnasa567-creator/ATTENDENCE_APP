...
from face_recognition_module import face_bp
from analytics_module        import analytics_bp
...
def teacher_dashboard():
    ...
    return render_template('teacher_dashboard.html', subjects=subjects, username=session['username'])
...
def mark_attendance():
    ...
    for student in students:
        ...
        if existing:
            cursor.execute("UPDATE attendance SET status=%s WHERE roll_no=%s AND subject_id=%s AND date=%s", (status, roll_no, subject_id, date))
        else:
            cursor.execute("INSERT INTO attendance (roll_no, subject_id, date, status) VALUES (%s,%s,%s,%s)", (roll_no, subject_id, date, status))
    db.commit()
    return redirect('/teacher/dashboard')
...
def student_dashboard():
    ...
    return render_template('student_dashboard.html', name=session['name'], roll_no=roll_no, stats=stats, timetable=timetable)
